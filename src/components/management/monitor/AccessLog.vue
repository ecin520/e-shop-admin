    ...
    <el-card v-loading="loading">
      ...
      <el-table size="small" :data="webLogData">
        ...
        <el-table-column align="center" label="结果" prop="result">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="showResultDetail(scope.$index, scope.row)">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination background style="float: right; min-height: 50px; margin-top: 20px;" layout="prev, pager, next"
                     @current-change="currentChange" :total="total" :page-size="pageSize"
...
<script>
  import {listWebLogs, listWebLogsByDate} from "../../../api/log";
  ...
  export default {
    ...
    methods: {
      ...
      changeDate() {
        this.loading = true;
        listWebLogsByDate(this.logDate).then(response => {
          this.pageWebLogData = response.data;
      ...
      init() {
        ...
        this.$store.dispatch("changeMenu", {hover: '7-1'});

        this.loading = true;
        listWebLogs().then(response => {
          this.pageWebLogData = response.data;
          this.total = this.pageWebLogData.length;
          if (this.total < this.pageSize) {
            this.webLogData = this.pageWebLogData;
