        ...
        <div style="display: flex;justify-content: center">
          ...
          >
            ...
            <el-form-item label="商品名称">
              <el-input style="width: 434px" v-model="product.name"></el-input>
            </el-form-item>
            <br>
            <el-form-item label="货号">
              <el-input disabled style="width: 434px" v-model="product.itemNo"></el-input>
            </el-form-item>
            <br>
            <el-form-item label="商品价格">
              <el-input style="width: 434px" v-model="product.price"></el-input>
            </el-form-item>
            <br>
            <el-form-item label="出售时间">
              <el-date-picker
                v-model="product.saleTime"
                type="datetime"
            ...
            <el-form-item label="成长值">
              <el-input style="width: 434px" v-model="product.growthValue"></el-input>
            </el-form-item>
            <br>
            <el-form-item label="赠送积分">
              <el-input style="width: 434px" v-model="product.integral"></el-input>
            </el-form-item>
            <br>
          ...
          <div style="width: 800px">
            <div style="border: 1px solid #DCDFE6;padding: 15px;">
              <el-row style="margin: 10px">
                <span>选择分类</span>
                <el-select disabled size="small" @change="changeProductCategory" v-model="productCategoryId" placeholder="请选择分类">
                  <el-option
        ...
        <div style="display: flex;justify-content: center;">
          <div style="width: 812px">
            <div style="border: 1px solid #DCDFE6;padding: 15px;">
              ...
              <div>
                <p>文案编辑：</p>
                <mark-editor v-model="product.description" :isClear="isClear"></mark-editor>
              </div>
            </div>
...
<script>
  import {
    ...
    listProductSpecDTOsByCategoryId, getProductCategoryDetailDTOById,
    listProductSpecDTOsByCategoryIdAndProductId, listSpecDetailsByProductId, listProductCategoryDetailsByParam
  } from "../../../../api/category";
  import {getProduct, getSkuProductByParam, insertProductByParam, updateProductByParam} from "../../../../api/product";
  ...
  export default {
    ...
    methods: {
      ...
      addProductSpec(productSpec) {
        ...
        insertProductSpecDetail({productSpecId: productSpec.id, name: productSpec.addSpec}).then(response => {
          if (response.code === 200) {
            if (productSpec.details === null || productSpec.details === '') {
              productSpec.details = []
            }
      ...
      submit() {
        ...
        this.product.showImage = imgList

        // 商品参数列表添加到product
        this.product.parameter = JSON.stringify(this.parameter)
        this.product.shopId = this.shopId;
        this.product.productCategoryDetailId = this.productCategoryDetailId;
        this.product.shopName = this.shopName;


        this.productCategoryDetailList.forEach(item => {
          if (item.id === this.productCategoryDetailId) {
            this.product.productCategoryDetailName = item.name
          }
        })
      ...
      changeProductCategoryAction(value, flag) {
        this.productSkuList = []
        ...
        listProductSpecDTOsByCategoryIdAndProductId(value, this.productId).then(response => {
          ...
          let specDetailList = null

          listSpecDetailsByProductId(this.productId).then(response => {
            specDetailList = response.data;

            for (let i = 0; i < this.productSpecList.length; i++) {
              if (this.productSpecList[i].details === null || this.productSpecList[i].details === '') {
                continue;
              }
              for (let j = 0; j < this.productSpecList[i].details.length; j++) {
                this.productSpecList[i].details[j].checked = false;
                specDetailList.forEach(item => {
                  if (item.id === this.productSpecList[i].details[j].id) {
                    this.productSpecList[i].details[j].checked = true;
                  }
                })
              }
      ...
      changeProductCategory(value) {
        this.changeProductCategoryAction(value, true)
      },
      ...
      changeSpecDetail() {
        ...
        for (let i = 0; i < this.specList.length; i++) {
          ...
          let specDetailIds = [];
          for (let j = 0; j < this.specList[i].length; j++) {
            specDetailIds.push(this.specList[i][j].id);
          }
          // 通过商品id和规格详情id列表确定sku
          getSkuProductByParam(this.productId, specDetailIds).then(response => {
            let skuProduct = response.data
            if (skuProduct !== null && skuProduct !== '') {
      ...
      init() {
        ...
        this.$store.dispatch("changeBreadcrumb", {value: ['商品管理', '商品管理', '编辑商品']});
        ...
        getProduct({id: this.productId}).then(response => {
          ...
          if (this.product.showImage !== null && this.product.showImage !== "") {
            let imgList = this.product.showImage.split(",")
            imgList.forEach(item => {
              this.uploadImageList.push({url: item})
            })
          }

          getProductCategoryDetailDTOById(this.product.productCategoryDetailId).then(response => {
            if (response.code === 200) {
              this.productCategoryId = response.data.productCategory.id;
              this.productCategoryDetailId = response.data.id;
              this.changeProductCategoryAction(this.productCategoryId, false)
            }
          })
        })
